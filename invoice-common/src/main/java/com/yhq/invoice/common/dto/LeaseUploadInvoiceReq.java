...
/**
 * 不动产经营租赁（LEASE）发票上传特定请求：直接继承 {@link InvoiceRequest}，<b>冗余声明</b>公共字段 + 不动产经营租赁专属字段，
 ...
 * 因此自动丢弃基础版 / 不动产销售 / 建筑服务 / 货物运输的专属字段，序列化后严格对应不动产经营租赁报文。
...
@NoArgsConstructor
public class LeaseUploadInvoiceReq extends InvoiceRequest {

    public LeaseUploadInvoiceReq(CapabilityCode capabilityCode) {
     ...
     * 不动产经营租赁特定要素列表
     */
    private List<UploadInvoiceReq.BdcjyzlTdysItem> bdcjyzltdys;

    /**
     * 发票明细行项目（fpmxList 数组项）：不动产经营租赁公共行级字段（无差异化）。
     */
    @Getter
    @Setter
    ...
    /**
     * 差额扣除（cekcList 数组项）：不动产经营租赁公共差额扣除字段（无差异化）。
     */
    @Getter
    @Setter
    ...
    /**
     * 从最外层超集 req 转为不动产经营租赁特定 req（仅保留公共 + 不动产经营租赁专属字段）。
     */
    public static LeaseUploadInvoiceReq from(UploadInvoiceReq req) {
        if (req == null) {
            return null;
        }
        LeaseUploadInvoiceReq target = new LeaseUploadInvoiceReq();
