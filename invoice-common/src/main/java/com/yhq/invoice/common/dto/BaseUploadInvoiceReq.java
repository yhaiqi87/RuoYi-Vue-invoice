...
import com.yhq.invoice.common.enums.FpkjfsDm;
import com.yhq.invoice.common.enums.Fppz;
import com.yhq.invoice.common.enums.Lzfpbz;
...
/**
 * 基础版（BASE）发票上传特定请求：直接继承 {@link InvoiceRequest}，<b>冗余声明</b>公共字段 + 基础版专属字段，
 ...
 * 因此自动丢弃不动产销售 / 经营租赁 / 建筑服务 / 货物运输的专属字段，序列化后严格对应基础版报文。
...
@NoArgsConstructor
public class BaseUploadInvoiceReq extends InvoiceRequest {

    public BaseUploadInvoiceReq(CapabilityCode capabilityCode) {
    ...
    @EnumValue(enumClass = FpkjfsDm.class, message = "发票开具方式代码(fpkjfsDm)取值必须为 4(第三方平台开票) 或 5(自建平台开票)")
    private String fpkjfsDm;
    @NotBlank(message = "发票票种(fppz)为必填项")
    @Size(max = 2, message = "发票票种(fppz)长度不能超过 2")
    @EnumValue(enumClass = Fppz.class, message = "发票票种(fppz)取值必须为 01 或 02")
    private String fppz;
    ...
    @EnumValue(enumClass = Lzfpbz.class, message = "蓝字发票标志(lzfpbz)取值必须为 0(蓝字) 或 1(红字)")
    ...
    /**
     * 涉税专业服务协议编号（varchar，20，否）
     */
    private String sszyfwxybh;
    /**
     * 是否开具涉税专业服务发票品目（varchar，1，否）
     */
    ...
    /**
     * 发票明细行项目（fpmxList 数组项）：基础版公共行级字段（无差异化）。
     */
    @Getter
    @Setter
    ...
    /**
     * 差额扣除（cekcList 数组项）：基础版公共差额扣除字段（无差异化）。
     */
    @Getter
    @Setter
    public static class CekcItem {
         ...
         */
        @DatePattern(pattern = "yyyy-MM-dd", message = "差额扣除开具日期(kjrq)格式必须为 yyyy-MM-dd")
        private String kjrq;
        /**
    ...
    /**
     * 从最外层超集 req 转为基础版特定 req（仅保留公共 + 基础版专属字段）。
     */
    public static BaseUploadInvoiceReq from(UploadInvoiceReq req) {
        if (req == null) {
            return null;
        }
        BaseUploadInvoiceReq target = new BaseUploadInvoiceReq();
