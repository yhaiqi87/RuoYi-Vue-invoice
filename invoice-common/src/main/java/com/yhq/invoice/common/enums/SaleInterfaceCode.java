...
/**
 * 不动产销售（乐企能力编码 202046）接口清单。
 *
 * <p>服务编码（serviceCode）取自《乐企数字化电子发票（不动产销售）开票能力说明文档-V3.004》的“接口清单”，
 * 对应请求头 {@code fwbm} 的取值；interfaceName 为对应“接口名称”。
 * 仅不动产销售支持：查询房源信息；另含批量发票下载申请。
 */
@Getter
@AllArgsConstructor
public enum SaleInterfaceCode {
    ...
    QUERY_RED_CONFIRM_LIST("CXQDHZQRDLB", "查询不动产销售红字确认单列表信息"),
    QUERY_RED_CONFIRM_DETAIL("CXQDHZQRDMX", "查询不动产销售红字确认单明细信息"),
    UPLOAD_INVOICE("QDFPSC_BDCXS", "不动产销售发票上传"),
    QUERY_UPLOAD_RESULT("CXQDFPSCJG", "查询不动产销售发票上传结果"),
    APPLY_RED_CONFIRM("QDHZQRDSQ", "不动产销售红字确认单申请"),
    CONFIRM_RED_CONFIRM("QDHZQRDQR", "不动产销售红字确认单确认"),
    QUERY_HOUSE_SOURCE("CXFYXX", "查询房源信息"),
    ...
    /** 按服务编码反查接口。 */
    public static SaleInterfaceCode fromServiceCode(String serviceCode) {
        for (SaleInterfaceCode code : values()) {
            if (code.serviceCode.equals(serviceCode)) {
                return code;
            }
        }
        throw new InvoiceException("未知的不动产销售接口服务编码: " + serviceCode);
    }
}
