...
/**
 * 商品条码（BARCODE）发票上传特定请求：直接继承 {@link InvoiceRequest}，<b>冗余声明</b>公共字段 + 商品条码专属字段，
 * 成为真正的能力子集（与基础版 BaseUploadInvoiceReq 同构，商品条码为通用数电票能力，无不动产 / 建筑等专属字段）。
 ...
 * 因此自动丢弃其它能力的专属字段，序列化后严格对应商品条码报文。类级 {@code @JsonInclude(NON_NULL)} 使空字段不输出。
 *
 * <p>注意：{@link FpmxItem#spBarcode} 为商品条码能力专属行级字段的<b>占位示例</b>，
 * 真实字段名 / 约束待《乐企数字化电子发票（商品条码）开票能力说明文档》补充后调整。
 */
@Getter
@Setter
@JsonInclude(JsonInclude.Include.NON_NULL)
@NoArgsConstructor
public class BarcodeUploadInvoiceReq extends InvoiceRequest {

    public BarcodeUploadInvoiceReq(CapabilityCode capabilityCode) {
    ...
    /**
     * 项目代码（varchar，6，否，通用数电票共有）。
     */
    private String xmDm;

    /**
     * 从最外层超集 req 转为商品条码特定 req（仅保留公共 + 商品条码专属字段）。
     */
    public static BarcodeUploadInvoiceReq from(UploadInvoiceReq req) {
        if (req == null) {
            return null;
        }
        BarcodeUploadInvoiceReq target = new BarcodeUploadInvoiceReq();
        BeanUtils.copyProperties(req, target);
        return target;
    }

    /**
     * 发票明细行项目（fpmxList 数组项）：通用数电票公共行级字段 + 商品条码差异化字段 spBarcode。
     */
    @Getter
    @Setter
    public static class FpmxItem {
        ...
        /**
         * 商品条码（占位字段）：商品条码能力专属行级字段示例，真实字段名 / 约束待乐企文档补充后调整。
         */
        private String spBarcode;
    }

    /**
     * 差额扣除（cekcList 数组项）：通用数电票公共差额扣除字段（无差异化）。
     */
    @Getter
    @Setter
