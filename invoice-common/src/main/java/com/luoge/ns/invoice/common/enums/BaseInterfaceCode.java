...
/**
 * 基础版（乐企能力编码 202007）接口清单。
 *
 * <p>服务编码（serviceCode）取自《乐企数字化电子发票（基础版）开票能力说明文档-V6.006》的“接口清单”，
 * 对应请求头 {@code fwbm} 的取值；interfaceName 为对应“接口名称”。
 * 仅基础版支持：查询涉税专业服务相关信息、原差额征税业务授权信息查询。
 */
@Getter
@AllArgsConstructor
public enum BaseInterfaceCode {
    ...
    /** 按服务编码反查接口。 */
    public static BaseInterfaceCode fromServiceCode(String serviceCode) {
        for (BaseInterfaceCode code : values()) {
            if (code.serviceCode.equals(serviceCode)) {
                return code;
            }
        }
        throw new InvoiceException("未知的基础版接口服务编码: " + serviceCode);
    }
}
