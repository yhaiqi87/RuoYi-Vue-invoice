...
/**
 * 货物运输（FREIGHT）发票上传特定请求：直接继承 {@link InvoiceRequest}，<b>冗余声明</b>公共字段（原公共基类
 * {@code AbstractUploadInvoiceReq} 已移除）。本能力单独定义行级 item 类 {@link FpmxItem}（公共行级字段 +
 * 货物运输差异化字段 ysmxxh/qyd/ddd/ysgjzl/ysgjph/yshwmc）、{@link CekcItem}（公共差额扣除字段 +
 * 货物运输差异化字段 fpdm/pzlx），附加要素 / 支付信息 item 类引用超集 {@link UploadInvoiceReq} 的嵌套定义。
 *
 * <p>由 {@link #from(UploadInvoiceReq)} 从最外层超集 req 拷贝字段得到： BeanUtils 仅拷贝目标类型声明的属性，
 * 因此自动丢弃其它能力的专属字段，序列化后严格对应货物运输报文。类级 {@code @JsonInclude(NON_NULL)} 使空字段不输出。
 */
@Getter
@Setter
@JsonInclude(JsonInclude.Include.NON_NULL)
@NoArgsConstructor
public class FreightUploadInvoiceReq extends InvoiceRequest {

    public FreightUploadInvoiceReq(CapabilityCode capabilityCode) {
    ...
    /**
     * 发票明细行项目（fpmxList 数组项）：货物运输公共行级字段 + 差异化字段
     * ysmxxh（运输明细序号）、qyd（起运地）、ddd（到达地）、ysgjzl（运输工具种类）、
     * ysgjph（运输工具牌号）、yshwmc（运输货物名称）。
     */
    @Getter
    @Setter
    ...
    /**
     * 差额扣除（cekcList 数组项）：货物运输公共差额扣除字段 + 差异化字段
     * fpdm（发票代码）、pzlx（凭证类型）。
     */
    @Getter
    @Setter
    ...
    /**
     * 从最外层超集 req 转为货物运输特定 req（仅保留公共字段，序列化严格对应货物运输报文）。
     */
    public static FreightUploadInvoiceReq from(UploadInvoiceReq req) {
        if (req == null) {
            return null;
        }
        FreightUploadInvoiceReq target = new FreightUploadInvoiceReq();
