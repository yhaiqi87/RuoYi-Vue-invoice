...
/**
 * 不动产经营租赁（乐企能力编码 202038）接口清单。
 *
 * <p>服务编码（serviceCode）取自《乐企数字化电子发票（不动产经营租赁）开票能力说明文档-V3.021》的“接口清单”，
 * 对应请求头 {@code fwbm} 的取值；interfaceName 为对应“接口名称”。
 * 含批量发票下载申请，但不含查询房源信息（仅不动产销售支持）。
 */
@Getter
@AllArgsConstructor
public enum LeaseInterfaceCode {
    ...
    QUERY_RED_CONFIRM_LIST("CXQDHZQRDLB", "查询不动产经营租赁红字确认单列表信息"),
    QUERY_RED_CONFIRM_DETAIL("CXQDHZQRDMX", "查询不动产经营租赁红字确认单明细信息"),
    UPLOAD_INVOICE("BDCJYZLFPSC", "不动产经营租赁发票上传"),
    QUERY_UPLOAD_RESULT("CXQDFPSCJG", "查询不动产经营租赁发票上传结果"),
    APPLY_RED_CONFIRM("QDHZQRDSQ", "不动产经营租赁红字确认单申请"),
    CONFIRM_RED_CONFIRM("QDHZQRDQR", "不动产经营租赁红字确认单确认"),
    ...
    /** 按服务编码反查接口。 */
    public static LeaseInterfaceCode fromServiceCode(String serviceCode) {
        for (LeaseInterfaceCode code : values()) {
            if (code.serviceCode.equals(serviceCode)) {
                return code;
            }
        }
        throw new InvoiceException("未知的不动产经营租赁接口服务编码: " + serviceCode);
    }
}
