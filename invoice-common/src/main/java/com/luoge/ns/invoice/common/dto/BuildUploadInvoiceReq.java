...
/**
 * 建筑服务（BUILD）发票上传特定请求：直接继承 {@link InvoiceRequest}，<b>冗余声明</b>公共字段 + 建筑服务专属字段，
 * 成为真正的能力子集（原公共基类 {@code AbstractUploadInvoiceReq} 已移除）。
 * 本能力单独定义行级 item 类 {@link FpmxItem}（在公共行级字段基础上补充建筑服务差异化字段 jzfwfsd/jzxmmc）、
 * {@link CekcItem}（公共差额扣除字段，无差异化），附加要素 / 支付信息 item 类引用超集 {@link UploadInvoiceReq} 的嵌套定义。
 *
 * <p>由 {@link #from(UploadInvoiceReq)} 从最外层超集 req 拷贝字段得到： BeanUtils 仅拷贝目标类型声明的属性，
 * 因此自动丢弃其它能力的专属字段，序列化后严格对应建筑服务报文。类级 {@code @JsonInclude(NON_NULL)} 使空字段不输出。
 */
@Getter
@Setter
@JsonInclude(JsonInclude.Include.NON_NULL)
@NoArgsConstructor
public class BuildUploadInvoiceReq extends InvoiceRequest {

    public BuildUploadInvoiceReq(CapabilityCode capabilityCode) {
    ...
    /**
     * 项目代码（varchar，6，否，基础版 / 不动产销售 / 建筑服务共有）
     */
    private String xmDm;
     ...
     * 建筑服务特定要素（仅建筑服务）
     */
    private UploadInvoiceReq.JzfwTdysItem jzfwtdys;

    /**
     * 发票明细行项目（fpmxList 数组项）：建筑服务公共行级字段 + 差异化字段
     * jzfwfsd（建筑服务发生地）、jzxmmc（建筑项目名称）。
     */
    @Getter
    @Setter
    ...
    /**
     * 差额扣除（cekcList 数组项）：建筑服务公共差额扣除字段（无差异化）。
     */
    @Getter
    @Setter
    ...
    /**
     * 从最外层超集 req 转为建筑服务特定 req（仅保留公共 + 建筑服务专属字段）。
     */
    public static BuildUploadInvoiceReq from(UploadInvoiceReq req) {
        if (req == null) {
            return null;
        }
        BuildUploadInvoiceReq target = new BuildUploadInvoiceReq();
